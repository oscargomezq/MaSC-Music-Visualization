...
namefile = open('namesfile.csv', 'w', encoding='utf-8')

for root, dirs, files in os.walk('.'):
    p = root
    for name in files:
        file_path =  (p+'/'+name)
        if ((".wav" in file_path) or (".aif" in file_path)):
            if ('EastAfricanClips' in file_path or 'Clips' in file_path):
                local_names.append(file_path)
                namefile.write(commas(file_path))
                namefile.write('\n')    
...
for p in range(1,8):
    ...
    fp = open('2d_Merged_Repaired_no_dups_mfcc_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_v2stdscale.csv', 'r', encoding='utf-8')
    x = []
    y = []
    ...
    output_file('2d_Merged_Repaired_no_dups_mfcc_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_v2_newstuff3stdscale.html')
