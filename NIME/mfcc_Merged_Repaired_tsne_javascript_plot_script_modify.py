...
for p in range(1,2):
    ...
    # model = TSNE(n_components=2, perplexity=prx, learning_rate=l_rate, n_iter=5000)#, init='pca'
    # np.set_printoptions(suppress=True)
    # T = model.fit_transform(X)
    # print (T.shape)
    # print (model.kl_divergence_)

    # fp = open('2d_Merged_Repaired_no_dups_mfcc_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_v2stdscale.csv', 'w', encoding='utf-8')
    # x = []
    # y = []
    # for i in range (samples):
    #     fp.write(names[i])
    ...
    df = pd.read_csv('2d_Merged_Repaired_no_dups_mfcc_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_v2stdscale.csv',
    ...
    df.to_csv('replicate_2d_Merged_Repaired_no_dups_mfcc_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_v2stdscale.csv',
               index=False, header=False, encoding='utf-8')

    fp = open('replicate_2d_Merged_Repaired_no_dups_mfcc_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_v2stdscale.csv', 'r', encoding='utf-8')
    x = []
    y = []
    labls = []
    ...
    output_file('replicate_2d_Merged_Repaired_no_dups_mfcc_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_v2_newstuff3stdscale.html')
