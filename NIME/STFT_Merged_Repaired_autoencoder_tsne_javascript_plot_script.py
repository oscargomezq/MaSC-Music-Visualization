...
filename = str(features) + 'd_Merged_Repaired_no_dups_STFT_autoencoder_try1.csv'
...
for p in range(1,2):
    ...
    # model = TSNE(n_components=2, perplexity=prx, learning_rate=l_rate, n_iter=2000)#, init='pca'
    # np.set_printoptions(suppress=True)
    # T = model.fit_transform(X)
    # print (T.shape)
    # print (model.kl_divergence_)

    # fp = open('50d_Merged_Repaired_no_dups_STFT_autoencoder_try1_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_stdscale1.csv', 'w', encoding='utf-8')
    # x = []
    # y = []
    # for i in range (samples):
    #     fp.write(names[i])
    ...
    df = pd.read_csv('50d_Merged_Repaired_no_dups_STFT_autoencoder_try1_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_stdscale1.csv',
    ...
    df.to_csv('replicate_50d_Merged_Repaired_no_dups_STFT_autoencoder_try1_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_stdscale1.csv',
               index=False, header=False, encoding='utf-8')


    fp = open('replicate_50d_Merged_Repaired_no_dups_STFT_autoencoder_try1_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_stdscale1.csv', 'r', encoding='utf-8')
    x = []
    y = []
    labls = []
    ...
    output_file('replicate_50d_Merged_Repaired_no_dups_STFT_autoencoder_try1_tsne_prx_' + str(prx) + '_l-rate_' + str(l_rate) + '_try_' + str(p) +'_newstuff3_KMeans50stdscale1.html')
    ...
    p1 = figure(tools=[hover, lasso_select, "reset", tap, wheel_zoom, box, "pan", help_b], toolbar_location="right", toolbar_sticky=False, title="Music Collections", width = 700, height = 700)
    ...
    #layout = row(children=[p1, None])#, sizing_mode='scale_height')#, p2)
    #l= layout ([p1])
    show(grid)
